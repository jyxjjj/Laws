...
$text = file_get_contents('original/快递市场管理办法.txt');
...
foreach ($text as $item) {
    ...
    if ($isXuyan) {
        ...
        $newText[] = "<Typography id='$hash' variant='title4'>$item</Typography>";
        continue;
    }
    ...
    if ($isZhang) {
        $hash = substr(sha512($item), 0, 8);
        $hashes[] = "<Typography><IDLink onClick={routeTo('$hash')}>$item</IDLink></Typography>";
        $newText[] = "<Typography id='$hash' variant='title4'>第$matches[1]章 $matches[2]</Typography>";
        continue;
    }
    ...
    if ($isJie) {
        $hash = substr(sha512($item), 0, 8);
        $hashes[] = "<Typography><IDLink onClick={routeTo('$hash')}>$item</IDLink></Typography>";
        $newText[] = "<Typography id='$hash' variant='title5'>第$matches[1]节 $matches[2]</Typography>";
        continue;
    }
